...
std::string LuaFName::to_string() const {
    if (!cached_string.empty()) return cached_string;
...
void register_fstring(sol::state& lua) {
    lua.new_usertype<LuaFString>("FString",
        sol::constructors<LuaFString(), LuaFString(const std::string&)>(),

        "ToString", &LuaFString::to_string,
        "Empty", &LuaFString::empty,
        "Clear", &LuaFString::empty, // alias
        "Len", &LuaFString::len,
        "IsEmpty", &LuaFString::is_empty,
        "Append", &LuaFString::append,
        "Find", [](const LuaFString& self, const std::string& search, sol::this_state ts) -> sol::object {
            sol::state_view lua(ts);
            int idx = self.find(search);
            if (idx < 0) return sol::nil;
            return sol::make_object(lua, idx + 1); // 1-based
        },
        "StartsWith", &LuaFString::starts_with,
        "EndsWith", &LuaFString::ends_with,
        "ToUpper", &LuaFString::to_upper,
        "ToLower", &LuaFString::to_lower,

        sol::meta_function::to_string, &LuaFString::to_string,
        sol::meta_function::length, &LuaFString::len,
        sol::meta_function::concatenation, [](const LuaFString& a, const LuaFString& b) -> LuaFString {
            LuaFString result(a.data);
            result.append(b.data);
            return result;
        },
        sol::meta_function::equal_to, [](const LuaFString& a, const LuaFString& b) -> bool {
...
LuaFString LuaFString::to_upper() const {
    ...
    for (char c : data) result.data += static_cast<char>(toupper(static_cast<unsigned char>(c)));
    return result;
}
LuaFString LuaFString::to_lower() const {
    LuaFString result;
    result.data.reserve(data.size());
    for (char c : data) result.data += static_cast<char>(tolower(static_cast<unsigned char>(c)));
    return result;
}
...
std::string LuaRemoteUnrealParam::type_name() const {
    switch (param_type) {
        case ParamType::UObjectPtr: return "UObject";
        case ParamType::Bool: return "bool";
        case ParamType::Int32: return "int32";
        case ParamType::Int64: return "int64";
        case ParamType::Float: return "float";
        case ParamType::Double: return "double";
        case ParamType::FNameVal: return "FName";
        case ParamType::FStringVal: return "FString";
        case ParamType::Byte: return "byte";
        case ParamType::RawPtr: return "pointer";
...
void register_thread_id(sol::state& lua) {
    lua.new_usertype<LuaThreadId>("ThreadId",
        sol::no_constructor,
        "ToString", [](const LuaThreadId& self) -> std::string {
            std::ostringstream oss;
            oss << self.id;
            return oss.str();
        },
        sol::meta_function::to_string, [](const LuaThreadId& self) -> std::string {
            std::ostringstream oss;
            oss << self.id;
            return oss.str();
        },
        sol::meta_function::equal_to, [](const LuaThreadId& a, const LuaThreadId& b) -> bool {
            return a.id == b.id;
        }
    );
}
