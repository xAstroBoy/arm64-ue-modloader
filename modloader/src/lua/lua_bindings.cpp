...
static NativeHookSig parse_native_hook_sig(const std::string& sig) {
    ...
    for (char c : args_part) {
        if (c == 'p' || c == 'i') result.arg_types.push_back('p');
        else if (c == 'f' || c == 'd') result.arg_types.push_back('f');
    }
    return result;
}
...
static std::string normalize_hook_path(const std::string& raw) {
    ...
    if (raw[0] != '/') return raw;                       // already short
    if (raw.compare(0, 8, "/Script/") != 0 &&
        raw.compare(0, 6, "/Game/")  != 0)  return raw;  // unknown prefix
...
static std::string normalize_class_name(const std::string& raw) {
    if (raw.empty() || raw[0] != '/') return raw;        // already short
    if (raw.compare(0, 8, "/Script/") != 0 &&
        raw.compare(0, 6, "/Game/")  != 0)  return raw;
    // Strip everything before the last '.'
    size_t dot = raw.rfind('.');
    if (dot != std::string::npos) return raw.substr(dot + 1);
...
static sol::object call_native_impl(sol::this_state ts, void* addr,
    ...
    if (!is_float_ret) {
        switch (x_idx) {
            ...
            case 8: result_int = ((fn_8)addr)(x_regs[0], x_regs[1], x_regs[2], x_regs[3], x_regs[4], x_regs[5], x_regs[6], x_regs[7]); break;
    ...
    switch (ret_type) {
        case 'v': return sol::nil;
        case 'i': return sol::make_object(lua, static_cast<int32_t>(result_int));
        case 'u': return sol::make_object(lua, static_cast<uint32_t>(result_int));
        case 'b': return sol::make_object(lua, result_int != 0);
        case 'p': return sol::make_object(lua, sol::lightuserdata_value(reinterpret_cast<void*>(result_int)));
        case 'f': return sol::make_object(lua, static_cast<float>(result_float));
        case 'd': return sol::make_object(lua, result_float);
        default:  return sol::make_object(lua, sol::lightuserdata_value(reinterpret_cast<void*>(result_int)));
    }
}
...
void register_all(sol::state& lua) {
    ...
    lua.set_function("StaticFindObject", [](sol::this_state ts, const std::string& path) -> sol::object {
        sol::state_view lua(ts);
        if (!symbols::StaticFindObject) return sol::nil;
        // Convert narrow string to wide for UE4's char16_t API
        std::wstring wpath(path.begin(), path.end());
        ue::UObject* obj = symbols::StaticFindObject(nullptr, nullptr, wpath.c_str(), false);
        if (!obj) return sol::nil;
        return lua_uobject::wrap_or_nil(lua, obj);
    });

    lua.set_function("StaticLoadClass", [](sol::this_state ts, const std::string& raw_base,
    ...
    lua.set_function("StaticLoadObject", [](sol::this_state ts, const std::string& raw_base,
                                             const std::string& path) -> sol::object {
        sol::state_view lua(ts);
    ...
    lua.set_function("NotifyOnNewObject", [](const std::string& raw_class,
    ...
    // ── Object construction ─────────────────────────────────────────────
    lua.set_function("ConstructObject", [](sol::this_state ts, sol::object class_arg,
                                            sol::optional<sol::object> outer_arg,
                                            sol::optional<std::string> name_arg) -> sol::object {
        sol::state_view lua(ts);
    ...
    lua.set_function("CreateWidget", [](sol::this_state ts, sol::object class_arg,
                                         sol::optional<sol::object> owner_arg) -> sol::object {
        sol::state_view lua(ts);
        ...
        //    use StaticConstructObject (NewObject) instead of WBL::Create
        if (!is_user_widget) {
            logger::log_info("LUA", "CreateWidget: '%s' is not a UserWidget — using NewObject",
                              cls_name_str.c_str());

            if (!symbols::StaticConstructObject) {
                logger::log_error("LUA", "CreateWidget: StaticConstructObject not resolved");
                return sol::nil;
            }

            // Get outer — use transient package or world
            ue::UObject* outer = nullptr;
            if (symbols::GetTransientPackage) {
                outer = symbols::GetTransientPackage();
            }
            if (!outer && symbols::GWorld) {
                outer = *reinterpret_cast<ue::UObject**>(symbols::GWorld);
            }

            ue::FName fname_none = {0, 0};
            ue::UObject* created = symbols::StaticConstructObject(
                widget_cls, outer, fname_none, 0, 0, nullptr, false, nullptr, false);

            if (created && ue::is_valid_ptr(created)) {
                logger::log_info("LUA", "CreateWidget: created %s @ 0x%lX (NewObject)",
                                  cls_name_str.c_str(),
            ...
            logger::log_error("LUA", "CreateWidget: NewObject returned null for '%s'",
                              cls_name_str.c_str());
            return sol::nil;
        }
    ...
    lua.set_function("RegisterHook", [](sol::this_state ts, const std::string& raw_path,
                                         sol::function callback) -> std::tuple<uint64_t, uint64_t> {
        sol::state_view lua(ts);
    ...
    lua.set_function("RegisterPreHook", [](const std::string& raw_path,
                                             sol::function callback) -> uint64_t {
        std::string func_path = normalize_hook_path(raw_path);
        if (func_path != raw_path) {
    ...
    lua.set_function("RegisterPostHook", [](const std::string& raw_path,
                                              sol::function callback) -> uint64_t {
        std::string func_path = normalize_hook_path(raw_path);
        if (func_path != raw_path) {
    ...
    lua.set_function("RegisterNativeHook", [](const std::string& symbol_name,
                                                sol::object pre_obj,
                                                sol::object post_obj,
                                                sol::optional<std::string> sig_str) -> bool {
    ...
    lua.set_function("RegisterNativeHookAt", [](void* addr,
    ...
    lua.set_function("CallNative", [](sol::this_state ts, void* addr,
                                       const std::string& sig, sol::variadic_args va) -> sol::object {
        return call_native_impl(ts, addr, sig, va);
    });

    lua.set_function("CallNativeBySymbol", [](sol::this_state ts, const std::string& symbol,
                                                const std::string& sig, sol::variadic_args va) -> sol::object {
    ...
    lua.set_function("Resolve", [](sol::this_state ts, const std::string& name,
                                    sol::optional<int64_t> fallback_offset) -> sol::object {
        sol::state_view lua(ts);
        void* addr = nullptr;
    ...
    lua.set_function("DumpSDK", []() {
        sdk_gen::generate();
        logger::log_info("LUA", "SDK dump regenerated");
    });
    ...
    mod_config.set_function("Save", [](sol::this_state ts, const std::string& mod_name,
                                        sol::table data) -> bool {
