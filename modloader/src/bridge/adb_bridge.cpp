...
static std::string handle_list_mods() {
    ...
    for (const auto& m : mods) {
        json entry;
        entry["name"] = m.name;
        entry["status"] = (m.status == mod_loader::ModStatus::LOADED) ? "loaded" :
                          (m.status == mod_loader::ModStatus::ERRORED) ? "errored" : "failed";
        entry["errors"] = m.error_count;
        if (!m.error.empty()) entry["last_error"] = m.error;
...
static std::string handle_reload_mod(const json& payload) {
    if (!payload.contains("name")) return error_response("missing 'name' parameter");
...
static std::string handle_exec_lua(const json& payload) {
    if (!payload.contains("code")) return error_response("missing 'code' parameter");
    ...
    pe_hook::queue_game_thread([ctx, code]() {
        ctx->result = lua_engine::exec_string(code, "=adb");
        {
    ...
    // Wait up to 5 seconds for the game thread to process it
    {
        std::unique_lock<std::mutex> lk(ctx->mtx);
        if (!ctx->cv.wait_for(lk, std::chrono::seconds(5), [&]{ return ctx->done; })) {
            return error_response("exec_lua timed out (game thread did not process within 5s)");
        }
    }
...
static std::string handle_log_tail(const json& payload) {
    int lines = 50;
    if (payload.contains("lines")) lines = payload["lines"];
...
static std::string handle_get_stats() {
    ...
    for (const auto& fs : stats) total_calls += fs.call_count;
...
static std::string handle_find_object(const json& payload) {
    ...
    if (!obj) return error_response("object not found: " + name);
...
static std::string handle_find_class(const json& payload) {
    ...
    if (!cls) return error_response("class not found: " + name);
...
static std::string dispatch(const std::string& raw_json) {
    ...
    if (!cmd.contains("cmd")) return error_response("missing 'cmd' field");

    std::string command = cmd["cmd"];
    logger::log_info("ADB", "Command: %s", command.c_str());

    if (command == "list_mods")     return handle_list_mods();
    if (command == "reload_mod")    return handle_reload_mod(cmd);
    if (command == "load_mod")      return handle_load_mod(cmd);
    if (command == "exec_lua")      return handle_exec_lua(cmd);
    if (command == "list_hooks")    return handle_list_hooks();
    if (command == "dump_sdk")      return handle_dump_sdk();
    if (command == "redump")        return handle_dump_sdk();
    if (command == "mount_pak")     return handle_mount_pak(cmd);
    if (command == "list_paks")     return handle_list_paks();
    if (command == "log_tail")      return handle_log_tail(cmd);
    if (command == "get_stats")     return handle_get_stats();
    if (command == "find_object")   return handle_find_object(cmd);
    if (command == "find_class")    return handle_find_class(cmd);
    if (command == "object_count")  return handle_object_count();

    // ═══ PE Trace commands ═══════════════════════════════════════════════
    if (command == "pe_trace_start") {
        std::string filter;
        if (cmd.contains("filter")) filter = cmd["filter"].get<std::string>();
    ...
    if (command == "pe_trace_top") {
        int n = 50;
        if (cmd.contains("n")) n = cmd["n"].get<int>();
        if (cmd.contains("lines")) n = cmd["lines"].get<int>();
    ...
    if (command == "log_stream")    return ""; // sentinel
    ...
    if (command == "help") {
        ...
        for (auto &b : built) result.push_back(b);
        // then append custom
        {
            std::lock_guard<std::mutex> lock(s_command_mutex);
            for (const auto &pair : s_custom_commands) {
    ...
    // These callbacks invoke Lua functions — must run on game thread
    {
        ...
        if (handler) {
            ...
            std::unique_lock<std::mutex> lk(ctx->mtx);
            if (!ctx->cv.wait_for(lk, std::chrono::seconds(5), [&]{ return ctx->done; })) {
                return error_response("command '" + command + "' timed out (game thread did not process within 5s)");
            }
...
static void handle_client(int client_fd) {
    ...
    while (s_running) {
        ...
        if (!accumulated.empty() && accumulated.find('{') != std::string::npos
