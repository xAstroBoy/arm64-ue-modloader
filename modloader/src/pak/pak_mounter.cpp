...
// Hardcoded paks path for early hooks (before JNI paths::init)
static const char* EARLY_PAKS_PATH = "/storage/emulated/0/Android/data/com.Armature.VR4/files/paks";
...
// ═══ Mount custom PAKs from hardcoded path — used by EARLY hooks ════════
...
static void mount_custom_paks_early(void* pak_pf) {
    ...
    DIR* dir = opendir(EARLY_PAKS_PATH);
    if (!dir) {
        __android_log_print(ANDROID_LOG_WARN, "UEModLoader",
            "[PAK-EARLY] Paks dir not found: %s — creating", EARLY_PAKS_PATH);
        mkdir(EARLY_PAKS_PATH, 0755);
        return;
    }
    ...
    if (pak_files.empty()) {
        __android_log_print(ANDROID_LOG_INFO, "UEModLoader",
            "[PAK-EARLY] No custom PAK files in %s", EARLY_PAKS_PATH);
        return;
    }
    ...
    for (size_t i = 0; i < pak_files.size(); i++) {
        std::string full_path = std::string(EARLY_PAKS_PATH) + "/" + pak_files[i];
...
static void capture_instance(void* this_ptr, const char* source) {
    if (s_capture_done.load(std::memory_order_relaxed)) return;

    // Atomic compare-exchange to ensure only one capture
    bool expected = false;
    if (!s_capture_done.compare_exchange_strong(expected, true)) return;
...
void install_early_hooks(uintptr_t base, void* lib_handle) {
    if (s_early_hooks_installed.load()) return;

    __android_log_print(ANDROID_LOG_INFO, "UEModLoader",
        "[PAK-EARLY] Installing Dobby hooks BEFORE engine init (base=0x%lX)",
        (unsigned long)base);

    // Resolve the Mount symbol via dlsym — need it for the trampoline
    void* mount_addr = nullptr;
    if (lib_handle) {
        mount_addr = dlsym(lib_handle, "_ZN16FPakPlatformFile5MountEPKDsjS1_b");
        if (!mount_addr)
            mount_addr = dlsym(lib_handle, "_ZN18FPakPlatformFile5MountEPKcijPKS0_");
        if (!mount_addr)
            mount_addr = dlsym(lib_handle, "_ZN20FPakPlatformFile5MountEPKDsjS1_b");
    }
    if (!mount_addr) {
        // Fallback to hardcoded offset
        mount_addr = reinterpret_cast<void*>(base + 0x08056024);
        __android_log_print(ANDROID_LOG_WARN, "UEModLoader",
            "[PAK-EARLY] Mount dlsym failed — using fallback 0x%lX",
            (unsigned long)reinterpret_cast<uintptr_t>(mount_addr));
    } else {
        __android_log_print(ANDROID_LOG_INFO, "UEModLoader",
            "[PAK-EARLY] Mount resolved via dlsym @ 0x%lX",
            (unsigned long)reinterpret_cast<uintptr_t>(mount_addr));
    }
    ...
    if (status == 0) {
        __android_log_print(ANDROID_LOG_INFO, "UEModLoader",
            "[PAK-EARLY] Hook 1/3: Mount hooked @ 0x%lX",
            (unsigned long)reinterpret_cast<uintptr_t>(mount_addr));
    } else {
        __android_log_print(ANDROID_LOG_ERROR, "UEModLoader",
            "[PAK-EARLY] Hook 1/3: Dobby FAILED on Mount (status=%d)", status);
    }

    // Hook 2: MountAllPakFiles #1 at offset 0x08054B78
    void* mount_all1_addr = reinterpret_cast<void*>(base + 0x08054B78);
    status = DobbyHook(
        mount_all1_addr,
    ...
    if (status == 0) {
        __android_log_print(ANDROID_LOG_INFO, "UEModLoader",
            "[PAK-EARLY] Hook 2/3: MountAllPakFiles#1 hooked @ 0x%lX",
            (unsigned long)reinterpret_cast<uintptr_t>(mount_all1_addr));
    } else {
        __android_log_print(ANDROID_LOG_ERROR, "UEModLoader",
            "[PAK-EARLY] Hook 2/3: Dobby FAILED on MountAllPakFiles#1 (status=%d)", status);
    }

    // Hook 3: MountAllPakFiles #2 at offset 0x08054FE8
    void* mount_all2_addr = reinterpret_cast<void*>(base + 0x08054FE8);
    status = DobbyHook(
        mount_all2_addr,
    ...
    if (status == 0) {
        __android_log_print(ANDROID_LOG_INFO, "UEModLoader",
            "[PAK-EARLY] Hook 3/3: MountAllPakFiles#2 hooked @ 0x%lX",
            (unsigned long)reinterpret_cast<uintptr_t>(mount_all2_addr));
    } else {
        __android_log_print(ANDROID_LOG_ERROR, "UEModLoader",
            "[PAK-EARLY] Hook 3/3: Dobby FAILED on MountAllPakFiles#2 (status=%d)", status);
    }
...
void install_hooks() {
    if (s_early_hooks_installed.load()) {
        ...
        if (!s_original_mount_all1) {
            ...
            if (status == 0) {
                logger::log_info("PAK", "Hook 2/3: MountAllPakFiles#1 hooked @ 0x%lX",
        ...
        if (!s_original_mount_all2) {
            ...
            if (status == 0) {
                logger::log_info("PAK", "Hook 3/3: MountAllPakFiles#2 hooked @ 0x%lX",
...
static void* get_pak_platform_file() {
    ...
    if (s_pak_platform_file) return s_pak_platform_file;
...
static ue::PakMountFn get_mount_fn() {
    if (s_original_mount) return s_original_mount;
...
bool is_mounted(const std::string& pak_name) {
    std::lock_guard<std::mutex> lock(s_pak_mutex);
    for (const auto& p : s_mounted) {
        if (p.name == pak_name && p.mounted) return true;
    }
    return false;
}
