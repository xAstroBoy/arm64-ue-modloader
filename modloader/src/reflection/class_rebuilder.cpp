...
ue::UObject* RebuiltClass::get_first_instance() {
    ...
    for (auto* obj : live_instances) {
        if (ue::is_valid_uobject(obj)) {
            ...
            if (!ue::is_default_object(name.c_str())) return obj;
        }
    }
    return nullptr;
}
...
ue::UObject* RebuiltClass::get_instance(int index) {
    ...
    int i = 0;
    for (auto* obj : live_instances) {
        if (!ue::is_valid_uobject(obj)) continue;
        std::string name = reflection::get_short_name(obj);
...
static RebuiltProperty convert_prop(const reflection::PropertyInfo& pi) {
    ...
    switch (pi.type) {
        case reflection::PropType::BoolProperty:   rp.type_name = "bool"; break;
        case reflection::PropType::FloatProperty:   rp.type_name = "float"; break;
        case reflection::PropType::DoubleProperty:  rp.type_name = "double"; break;
        case reflection::PropType::IntProperty:     rp.type_name = "int32"; break;
        case reflection::PropType::Int64Property:   rp.type_name = "int64"; break;
        case reflection::PropType::ByteProperty:    rp.type_name = "uint8"; break;
        case reflection::PropType::NameProperty:    rp.type_name = "FName"; break;
        case reflection::PropType::StrProperty:     rp.type_name = "FString"; break;
        case reflection::PropType::ObjectProperty:  rp.type_name = pi.inner_type_name.empty() ? "UObject*" : pi.inner_type_name; break;
        case reflection::PropType::StructProperty:  rp.type_name = pi.inner_type_name.empty() ? "struct" : pi.inner_type_name; break;
        case reflection::PropType::ArrayProperty:   rp.type_name = "TArray<" + pi.inner_type_name + ">"; break;
        case reflection::PropType::EnumProperty:    rp.type_name = pi.inner_type_name.empty() ? "enum" : pi.inner_type_name; break;
        default: rp.type_name = "unknown"; break;
    }
...
RebuiltClass* rebuild(const std::string& class_name) {
    ...
    // to avoid recursive mutex deadlock (std::mutex is NOT recursive).
    {
        ...
        if (it != s_classes.end()) return &it->second;
    } // Lock released here
        ...
        for (auto& pair : s_classes) {
            ...
            rc.prop_hooks.erase(
                std::remove_if(rc.prop_hooks.begin(), rc.prop_hooks.end(),
                    [id](const RebuiltClass::PropHookEntry& e) { return e.id == id; }),
                rc.prop_hooks.end());
            rc.func_hooks.erase(
                std::remove_if(rc.func_hooks.begin(), rc.func_hooks.end(),
                    [id](const RebuiltClass::FuncHookEntry& e) { return e.id == id; }),
                rc.func_hooks.end());
        }
    }
    ...
    // Remove from instance hooks
    {
        ...
        s_instance_hooks.erase(
            std::remove_if(s_instance_hooks.begin(), s_instance_hooks.end(),
                [id](const InstanceHook& ih) { return ih.id == id; }),
...
void tick(ue::UObject* self, ue::UFunction* func, void* parms) {
    ...
    if (!s_has_any_tracking.load(std::memory_order_relaxed)) return;

    if (!self || !ue::is_valid_ptr(self)) return;

    // Get the class name of the calling object
    ue::UClass* cls = ue::uobj_get_class(self);
    if (!cls || !ue::is_valid_ptr(cls)) return;
    ...
    if (ue::is_default_object(obj_name.c_str())) return;
    ...
    // Dispatch per-instance function hooks
    {
        ...
        for (auto& ih : s_instance_hooks) {
            if (ih.is_prop) continue;
            if (ih.obj != self) continue;
            if (ih.name != func_name) continue;
            if (ih.func_pre) {
...
ue::UObject* get_cdo(const std::string& class_name) {
    ...
    for (int32_t i = 0; i < count; i++) {
        ...
        if (!ue::is_valid_ptr(reinterpret_cast<const void*>(obj_objects))) continue;
        ...
        if (!ue::is_valid_ptr(reinterpret_cast<const void*>(chunk_ptr))) continue;
        obj = ue::read_field<ue::UObject*>(
            reinterpret_cast<const void*>(chunk_ptr + within * ue::FUOBJECTITEM_SIZE), 0);

        if (!obj || !ue::is_valid_ptr(obj)) continue;
        if (ue::uobj_get_class(obj) != cls) continue;
        std::string name = reflection::get_short_name(obj);
        if (name.find("Default__") == 0) return obj;
    }
    return nullptr;
}

bool is_valid_instance(ue::UObject* obj) {
    if (!ue::is_valid_uobject(obj)) return false;
    std::string name = reflection::get_short_name(obj);
