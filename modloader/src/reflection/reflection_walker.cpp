...
std::string fname_to_string(int32_t index) {
    if (index < 0 || s_fname_pool == 0) return "";
    ...
    if (block_idx < 0 || block_idx >= static_cast<int32_t>(ue::FNAME_MAX_BLOCKS)) return "";
    ...
    if (!ue::is_valid_ptr(reinterpret_cast<const void*>(block_ptr))) return "";

    // Compute entry address within block
    uintptr_t entry_addr = block_ptr + static_cast<uintptr_t>(offset_in_block) * ue::FNAME_STRIDE;
    if (!ue::is_valid_ptr(reinterpret_cast<const void*>(entry_addr))) return "";
...
// Walks the FNamePool to find a matching entry and returns its index.
// Results are cached in a static map for O(1) subsequent lookups.
...
int32_t fname_string_to_index(const std::string& name) {
    {
        ...
        if (it != s_fname_reverse_cache.end()) return it->second;
    }

    // If not cached, do a brute-force scan of the FName pool
    // This is expensive but only happens once per unique string
    if (s_fname_pool == 0) return 0;
    ...
    int32_t max_index = (current_block << ue::FNAME_BLOCK_BITS) + (current_byte / ue::FNAME_STRIDE);

    for (int32_t idx = 0; idx < max_index && idx < 500000; idx++) {
        std::string resolved = fname_to_string(idx);
        if (resolved == name) {
            std::lock_guard<std::mutex> lock(s_fname_reverse_mutex);
            s_fname_reverse_cache[name] = idx;
            return idx;
        }
    }
...
std::string get_package_name(const ue::UObject* obj) {
    ...
    while (current && ue::is_valid_ptr(current)) {
        ...
        if (!outer || !ue::is_valid_ptr(outer)) break;
        current = outer;
    }
...
PropType classify_property(const ue::FField* field) {
    ...
    if (cn.empty()) return PropType::Unknown;

    if (cn == "BoolProperty")     return PropType::BoolProperty;
    if (cn == "ByteProperty")     return PropType::ByteProperty;
    if (cn == "Int8Property")     return PropType::Int8Property;
    if (cn == "Int16Property")    return PropType::Int16Property;
    if (cn == "IntProperty")      return PropType::IntProperty;
    if (cn == "Int64Property")    return PropType::Int64Property;
    if (cn == "UInt16Property")   return PropType::UInt16Property;
    if (cn == "UInt32Property")   return PropType::UInt32Property;
    if (cn == "UInt64Property")   return PropType::UInt64Property;
    if (cn == "FloatProperty")    return PropType::FloatProperty;
    if (cn == "DoubleProperty")   return PropType::DoubleProperty;
    if (cn == "NameProperty")     return PropType::NameProperty;
    if (cn == "StrProperty")      return PropType::StrProperty;
    if (cn == "TextProperty")     return PropType::TextProperty;
    if (cn == "ObjectProperty")   return PropType::ObjectProperty;
    if (cn == "WeakObjectProperty")  return PropType::WeakObjectProperty;
    if (cn == "LazyObjectProperty")  return PropType::LazyObjectProperty;
    if (cn == "SoftObjectProperty")  return PropType::SoftObjectProperty;
    if (cn == "ClassProperty")    return PropType::ClassProperty;
    if (cn == "SoftClassProperty")return PropType::SoftClassProperty;
    if (cn == "InterfaceProperty")return PropType::InterfaceProperty;
    if (cn == "StructProperty")   return PropType::StructProperty;
    if (cn == "ArrayProperty")    return PropType::ArrayProperty;
    if (cn == "MapProperty")      return PropType::MapProperty;
    if (cn == "SetProperty")      return PropType::SetProperty;
    if (cn == "EnumProperty")     return PropType::EnumProperty;
    if (cn == "DelegateProperty") return PropType::DelegateProperty;
    if (cn == "MulticastDelegateProperty")       return PropType::MulticastDelegateProperty;
    if (cn == "MulticastInlineDelegateProperty")  return PropType::MulticastInlineDelegateProperty;
    if (cn == "MulticastSparseDelegateProperty")  return PropType::MulticastSparseDelegateProperty;
    if (cn == "FieldPathProperty")return PropType::FieldPathProperty;
...
static std::string prop_type_to_string(PropType pt) {
    switch (pt) {
        ...
        case PropType::ByteProperty:     return "uint8";
        case PropType::Int8Property:     return "int8";
        case PropType::Int16Property:    return "int16";
        case PropType::IntProperty:      return "int32";
        case PropType::Int64Property:    return "int64";
        case PropType::UInt16Property:   return "uint16";
        case PropType::UInt32Property:   return "uint32";
        case PropType::UInt64Property:   return "uint64";
        ...
        case PropType::StrProperty:      return "FString";
        case PropType::TextProperty:     return "FText";
        case PropType::ObjectProperty:   return "UObject*";
        case PropType::WeakObjectProperty: return "TWeakObjectPtr";
        case PropType::LazyObjectProperty: return "TLazyObjectPtr";
        case PropType::SoftObjectProperty: return "TSoftObjectPtr";
        case PropType::ClassProperty:    return "UClass*";
        case PropType::SoftClassProperty:return "TSoftClassPtr";
        case PropType::InterfaceProperty:return "FScriptInterface";
        case PropType::StructProperty:   return "struct";
        case PropType::ArrayProperty:    return "TArray";
        case PropType::MapProperty:      return "TMap";
        case PropType::SetProperty:      return "TSet";
        case PropType::EnumProperty:     return "enum";
        case PropType::DelegateProperty: return "FDelegate";
        case PropType::MulticastDelegateProperty:      return "FMulticastDelegate";
        case PropType::MulticastInlineDelegateProperty: return "FMulticastInlineDelegate";
        case PropType::MulticastSparseDelegateProperty: return "FMulticastSparseDelegate";
        case PropType::FieldPathProperty:return "FFieldPath";
...
static std::string get_inner_type_name(const ue::FProperty* prop, PropType pt) {
    switch (pt) {
        ...
        case PropType::SoftObjectProperty: {
            ue::UClass* inner = ue::read_field<ue::UClass*>(prop, ue::fprop::OBJ_PROPERTY_CLASS);
            if (ue::is_valid_ptr(inner)) {
                return get_short_name(reinterpret_cast<const ue::UObject*>(inner));
            }
            return "";
        }
        case PropType::ClassProperty: {
            ue::UClass* meta = ue::read_field<ue::UClass*>(prop, ue::fprop::CLASS_META_CLASS);
            if (ue::is_valid_ptr(meta)) {
                return get_short_name(reinterpret_cast<const ue::UObject*>(meta));
            }
            return "";
        }
        case PropType::StructProperty: {
            ue::UStruct* inner = ue::read_field<ue::UStruct*>(prop, ue::fprop::STRUCT_INNER_STRUCT);
            if (ue::is_valid_ptr(inner)) {
                return get_short_name(reinterpret_cast<const ue::UObject*>(inner));
            }
            return "";
        ...
        case PropType::EnumProperty: {
            ue::UEnum* inner = ue::read_field<ue::UEnum*>(prop, ue::fprop::ENUM_PROP_ENUM);
            if (ue::is_valid_ptr(inner)) {
                return get_short_name(reinterpret_cast<const ue::UObject*>(inner));
            }
            return "";
        ...
        case PropType::ArrayProperty: {
            ue::FProperty* inner = ue::read_field<ue::FProperty*>(prop, ue::fprop::ARRAY_INNER);
            if (ue::is_valid_ptr(inner)) {
...
std::vector<PropertyInfo> walk_properties(const ue::UStruct* ustruct, bool include_inherited) {
    ...
    while (current) {
        ...
        while (field && ue::is_valid_ptr(field)) {
            ...
            if (pt != PropType::Unknown) {
                ...
                if (pt == PropType::BoolProperty) {
                    pi.bool_field_size = ue::read_field<uint8_t>(field, ue::fprop::BOOL_FIELD_SIZE);
                    pi.bool_byte_offset = ue::read_field<uint8_t>(field, ue::fprop::BOOL_BYTE_OFFSET);
                    pi.bool_byte_mask = ue::read_field<uint8_t>(field, ue::fprop::BOOL_BYTE_MASK);
                    pi.bool_field_mask = ue::read_field<uint8_t>(field, ue::fprop::BOOL_FIELD_MASK);
                } else {
                ...
                if (pt == PropType::MapProperty) {
                    ue::FProperty* val_prop = ue::read_field<ue::FProperty*>(field, ue::fprop::MAP_VALUE_PROP);
                    if (ue::is_valid_ptr(val_prop)) {
        ...
        if (include_inherited) {
            current = ue::ustruct_get_super(current);
        } else {
            break;
        }
    }
...
static ue::UObject* get_object_at_index(int32_t index) {
    if (s_guobject_array == 0) return nullptr;
    ...
    if (!ue::is_valid_ptr(reinterpret_cast<const void*>(obj_objects))) return nullptr;
    ...
    if (!ue::is_valid_ptr(reinterpret_cast<const void*>(chunk_ptr))) return nullptr;

    // Read UObject* from within chunk: chunk_ptr + within_chunk * FUOBJECTITEM_SIZE
    uintptr_t item_addr = chunk_ptr + static_cast<uintptr_t>(within_chunk) * ue::FUOBJECTITEM_SIZE;
...
static int32_t get_num_elements() {
    if (s_guobject_array == 0) return 0;
    // FChunkedFixedUObjectArray is EMBEDDED at GUObjectArray + 0x10
...
static bool is_uclass(const ue::UObject* obj) {
    ...
    if (!cls || !ue::is_valid_ptr(cls)) return false;
    std::string class_name = get_short_name(reinterpret_cast<const ue::UObject*>(cls));
    return (class_name == "Class" || class_name == "BlueprintGeneratedClass"
...
void init() {
    ...
    if (symbols::GUObjectArray) {
        ...
        logger::log_info("REFLECT", "  Objects**: 0x%lX (%s)",
                         ...
                         ue::is_valid_ptr(reinterpret_cast<const void*>(obj_ptrs)) ? "valid" : "INVALID");
        logger::log_info("REFLECT", "  NumElements: %d", num_elements);
        logger::log_info("REFLECT", "  MaxElements(+4): %d", max_elements);

        // If Objects** is valid, read first chunk pointer
        if (ue::is_valid_ptr(reinterpret_cast<const void*>(obj_ptrs))) {
            ...
            // If chunk 0 is valid, read first object pointer
        ...
        // Must iterate packed entries by reading each header to compute the next valid index.
        {
            ...
            if (block0_ptr && ue::is_valid_ptr(reinterpret_cast<const void*>(block0_ptr))) {
                ...
                while (it < end && entry_count < max_test) {
                    ...
                    if (str_len <= 0) break;  // null terminator entry
                    if (str_len >= 250) break;  // sanity
...
void walk_all() {
    ...
    for (int32_t i = 0; i < s_object_count; i++) {
        ...
        if (!obj) { null_count++; continue; }
        if (!ue::is_valid_ptr(obj)) { invalid_count++; continue; }
        ...
        if (is_uclass(obj)) {
            ...
            ci.is_blueprint = (meta_name == "BlueprintGeneratedClass"
                            || meta_name == "WidgetBlueprintGeneratedClass"
                            || meta_name == "AnimBlueprintGeneratedClass");
            ...
            if (ue::is_valid_ptr(reinterpret_cast<const void*>(data_ptr)) && num_values > 0 && num_values < 10000) {
                for (int32_t j = 0; j < num_values; j++) {
                    ...
                    if (value_name.size() >= 4 && value_name.substr(value_name.size() - 4) == "_MAX") continue;
...
ClassInfo* find_class(const std::string& name) {
    ...
    if (it != s_class_map.end()) return &s_classes[it->second];
    return nullptr;
}

StructInfo* find_struct(const std::string& name) {
    auto it = s_struct_map.find(name);
    if (it != s_struct_map.end()) return &s_structs[it->second];
    return nullptr;
}

EnumInfo* find_enum(const std::string& name) {
    auto it = s_enum_map.find(name);
    if (it != s_enum_map.end()) return &s_enums[it->second];
    return nullptr;
}
...
ue::UClass* find_class_ptr(const std::string& name) {
    ...
    if (obj && is_uclass(obj)) return reinterpret_cast<ue::UClass*>(obj);
    return nullptr;
}
...
ue::UObject* find_first_instance(const std::string& class_name) {
    ...
    for (int32_t i = 0; i < count; i++) {
        ...
        // Skip CDOs
        std::string obj_name = get_short_name(obj);
        if (ue::is_default_object(obj_name.c_str())) continue;
        // Check class name
        ue::UClass* cls = ue::uobj_get_class(obj);
        if (!cls || !ue::is_valid_ptr(cls)) continue;
        std::string cls_name = get_short_name(reinterpret_cast<const ue::UObject*>(cls));
        if (cls_name == class_name) return obj;
    }
    return nullptr;
}
...
std::vector<ue::UObject*> find_all_instances(const std::string& class_name) {
    ...
    for (int32_t i = 0; i < count; i++) {
        ...
        if (!cls || !ue::is_valid_ptr(cls)) continue;
        std::string cls_name = get_short_name(reinterpret_cast<const ue::UObject*>(cls));
        if (cls_name == class_name) result.push_back(obj);
    }
    return result;
}
