...
void init(const std::string& log_path) {
    ...
    if (!s_file) {
        __android_log_print(ANDROID_LOG_ERROR, LOGCAT_TAG,
                            "Failed to open log file: %s (errno=%d)", log_path.c_str(), errno);
        return;
    }
...
static void write_line(const char* level, const char* source, const char* msg) {
    ...
    if (strcmp(level, "WARN") == 0) prio = ANDROID_LOG_WARN;
    else if (strcmp(level, "ERROR") == 0) prio = ANDROID_LOG_ERROR;
    else if (strcmp(level, "DEBUG") == 0) prio = ANDROID_LOG_DEBUG;
