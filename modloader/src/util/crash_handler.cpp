...
static bool pc_in_modloader(uintptr_t pc) {
    if (s_modloader_base == 0) return false;
...
static const char* signal_code_name(int sig, int code) {
    if (sig == SIGSEGV) {
        switch (code) {
            ...
            case SEGV_ACCERR: return "SEGV_ACCERR (access permission fault)";
            default: return "unknown code";
        }
    }
    if (sig == SIGBUS) {
        switch (code) {
            case BUS_ADRALN: return "BUS_ADRALN (alignment fault)";
...
static void crash_handler_fn(int sig, siginfo_t* info, void* ucontext_raw) {
    ...
    if (!is_our_crash) {
        ...
        switch (sig) {
            case SIGSEGV: old_action = &s_old_sigsegv; break;
            case SIGABRT: old_action = &s_old_sigabrt; break;
            case SIGBUS:  old_action = &s_old_sigbus;  break;
            case SIGFPE:  old_action = &s_old_sigfpe;  break;
        }
