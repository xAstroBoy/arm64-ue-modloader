...
// Find the on-disk path of libUE4.so from /proc/self/maps
...
static void init_elf_cache() {
    if (s_elf_init_done) return; // already initialized
    s_elf_init_done = true;

    std::string lib_path = find_lib_path_for_scan();
    if (lib_path.empty()) {
        logger::log_warn("RESOLVE", "Cannot find libUE4.so path for ELF cache");
        return;
    }
    ...
    for (int i = 0; i < ehdr->e_shnum; i++) {
        ...
        if (shdrs[i].sh_link >= static_cast<unsigned>(ehdr->e_shnum)) continue;
        const Elf64_Shdr* str_shdr = &shdrs[shdrs[i].sh_link];

        // Bounds check
        if (shdrs[i].sh_offset + shdrs[i].sh_size > s_elf_mmap_size) continue;
        if (str_shdr->sh_offset + str_shdr->sh_size > s_elf_mmap_size) continue;
    ...
    int total_syms = 0;
    for (auto& sec : s_elf_sections) total_syms += static_cast<int>(sec.num_syms);
    ...
    // Uses ~5MB vs ~40MB for full pre-demangling. Zero string allocation except method keys.
    {
        ...
        for (uint16_t si = 0; si < static_cast<uint16_t>(s_elf_sections.size()); si++) {
            ...
            for (uint32_t j = 0; j < static_cast<uint32_t>(sec.num_syms); j++) {
                ...
                if (sym.st_name == 0 || sym.st_name >= sec.strtab_size) continue;
                ...
                if (is_ue4_metadata_symbol(name)) continue;
                ...
                if (name[2] == 'N') {
                    ...
                    while (*p && *p != 'E' && count < 16) {
                        if (*p == 'C' && p[1] >= '1' && p[1] <= '3') { is_ctor = true; break; }
                        if (*p == 'D' && p[1] >= '0' && p[1] <= '2') { is_dtor = true; break; }
                        if (*p == 'S') { p++; if (*p == 't') { p++; continue; } while (*p && *p != '_') p++; if (*p == '_') p++; continue; }
                        if (*p == 'I') { int d=1; p++; while (*p && d>0) { if (*p=='I') d++; else if (*p=='E') d--; p++; } continue; }
                        if (!isdigit((unsigned char)*p)) break;
                        int len = 0;
                        while (isdigit((unsigned char)*p)) { len = len*10+(*p-'0'); p++; }
                        if (len <= 0 || len > 500) break;
                        ids[count] = p; lens[count] = len; count++;
                        p += len;
                    }
...
static bool is_ue4_metadata_symbol(const char* sym_name) {
    ...
    if (strstr(sym_name, "Z_Construct_U")) return true;
    // _Statics — static data containers inside registration functions
    if (strstr(sym_name, "_Statics")) return true;
    // NewProp_ — property metadata
    if (strstr(sym_name, "NewProp_")) return true;
    // PropPointers — property pointer arrays
    if (strstr(sym_name, "PropPointers")) return true;
    // FuncParams — function parameter metadata
    if (strstr(sym_name, "FuncParams")) return true;
    return false;
}
...
static void* elf_scan(const std::string& name) {
    init_elf_cache();
    if (s_elf_sections.empty() || s_lib_base == 0) return nullptr;
    ...
    if (name.size() > 4 && name[0] == '_' && name[1] == 'Z' && name[2] == 'N') {
        ...
        if (dm) {
            ...
            if (space != std::string::npos) prefix = prefix.substr(space + 1);
            ...
            while (*p && *p != 'E') {
                if (*p == 'C' || *p == 'D') break;  // ctor/dtor
                if (*p == 'S' || *p == 'I') break;  // substitution/template — stop
                if (!isdigit((unsigned char)*p)) break;
                int len = 0;
                while (isdigit((unsigned char)*p)) { len = len * 10 + (*p - '0'); p++; }
                ...
                for (int k = 0; k < len && *p && *p != 'E'; k++, p++) {
                    if (isdigit((unsigned char)*p) && k > 2) {
                        ...
                        const char* q = p;
                        int maybe_len = 0;
                        while (isdigit((unsigned char)*q)) { maybe_len = maybe_len * 10 + (*q - '0'); q++; }
        ...
        if (!method_hint.empty()) {
            ...
            if (idx_it != s_method_idx.end()) {
                ...
                struct Match { void* addr; std::string mangled; std::string demangled; };
                ...
                for (auto& ref : idx_it->second) {
                    ...
                    if (!class_hint.empty()) {
                        if (strstr(sym_name, class_hint.c_str()) == nullptr) continue;
                    }
                    int st;
                ...
                if (!matches.empty()) {
                    auto& best = *std::min_element(matches.begin(), matches.end(),
                        [](const Match& a, const Match& b) { return a.demangled.size() < b.demangled.size(); });
        ...
        if (search_str.find("::") != std::string::npos) {
            ...
            while (pos < search_str.size()) {
                ...
                if (next == std::string::npos) {
                    if (pos < search_str.size()) words.push_back(search_str.substr(pos));
                    break;
                }
                if (next > pos) words.push_back(search_str.substr(pos, next - pos));
                pos = next + 2;
            }
                ...
                if (!lookup_method.empty()) {
                    ...
                    if (idx_it != s_method_idx.end()) {
                        ...
                        for (auto& ref : idx_it->second) {
                            ...
                            int clen = 0;
                            while (isdigit((unsigned char)*p)) { clen = clen*10+(*p-'0'); p++; }
                            if (clen <= 0 || clen > 500) continue;
                            std::string cls_id(p, clen);

                            // Class must contain query class_part as substring
                            if (cls_id.find(class_part) == std::string::npos) continue;
                            ...
                            if (!is_dtor && !is_ctor && words.size() > 2) {
                                ...
                                for (size_t wi = 2; wi < words.size(); wi++) {
                                    if (!strstr(dm, words[wi].c_str())) { ok = false; break; }
                                }
                                free(dm);
                                if (!ok) continue;
                            }
    ...
    for (auto& sec : s_elf_sections) {
        for (size_t j = 0; j < sec.num_syms; j++) {
            ...
            if (strstr(sym_name, search) != nullptr) {
                if (is_ue4_metadata_symbol(sym_name)) continue;
    ...
    if (!candidates.empty()) {
        auto& best = *std::min_element(candidates.begin(), candidates.end(),
            [](const SubstringCandidate& a, const SubstringCandidate& b) {
                ...
                if (a_func != b_func) return a_func;
...
void init() {
    ...
    if (!s_lib_handle) {
        s_lib_handle = dlopen("libUE4.so", RTLD_NOW);
    }
    if (!s_lib_handle) {
        logger::log_error("SYMBOL", "Failed to open libUE4.so: %s", dlerror());
        return;
    }
    ...
    if (base_ctx.found) {
        ...
        logger::log_info("SYMBOL", "libUE4.so base: 0x%lX", s_lib_base);
    } else {
        // Fallback: use dlsym on a known symbol and subtract the known offset
        logger::log_warn("SYMBOL", "Could not find libUE4.so base via dl_iterate_phdr");
    }

    // Register UE4Dumper-derived fallback offsets for core symbols
    register_fallback("GUObjectArray", 0x0A785E08);
    register_fallback("GNames",        0x0A763300);

    // All offsets from Binary Ninja / UE4Dumper export analysis of libUE4.so
    register_fallback("StaticFindObject",                  0x06E1AE74);
    register_fallback("StaticLoadObject",                  0x06E1D42C);
    register_fallback("StaticLoadClass",                   0x06E1DE74);
    register_fallback("StaticConstructObject_Internal",    0x06E20A8C);
    register_fallback("GetEtcModelClass",                  0x066909C8);
    register_fallback("FPakPlatformFile::Mount",           0x08056024);

    // FPlatformFileManager — needed for PAK mounting to find FPakPlatformFile instance
    register_fallback("_ZN20FPlatformFileManager3GetEv",               0x0666D4B4);
    register_fallback("_ZN20FPlatformFileManager16FindPlatformFileEPKDs", 0x0666D034);

    // FText symbols — needed for TextProperty read/write/call support
    register_fallback("_ZNK5FText8ToStringEv",          0x066C49BC);  // FText::ToString() const
    register_fallback("_ZN5FText10FromStringEO7FString", 0x066C4FA8); // FText::FromString(FString&&)
    register_fallback("_ZN5FTextD2Ev",                   0x055089A8); // FText::~FText()
    register_fallback("_ZN5FTextC1Ev",                   0x066C3DE4); // FText::FText()
...
void resolve_core_symbols() {
    ...
    // StaticLoadClass — fallback offset registered in init()
    void* slc = resolve("StaticLoadClass");
    StaticLoadClass = reinterpret_cast<ue::StaticLoadClassFn>(slc);
    logger::log_info("SYMBOL", "StaticLoadClass: %s", slc ? "OK" : "NOT FOUND");
    ...
    if (!gtp) gtp = resolve("GetTransientPackage");
    ...
    if (!GNames) GNames = resolve("_ZL6GNames");
    if (!GNames) GNames = resolve("GNamePool");
    if (!GNames) GNames = resolve("NamePoolData");
    if (GNames) {
    ...
    if (!GetEtcModelClass) GetEtcModelClass = resolve("UVR4DataSingleton::GetEtcModelClass");
    ...
    if (!fni) fni = resolve("FName::FName");
    if (!fni) fni = resolve("_ZN5FNameC1EPKwi");
    if (!fni) fni = resolve("_ZN5FNameC2EPKwi");
    ...
    if (!fol) fol = resolve("_ZN13FOutputDevice3LogEPKw");
    ...
    int resolved = 0;
    int total = 17;
    if (ProcessEvent) resolved++;
    if (StaticFindObject) resolved++;
    if (StaticLoadObject) resolved++;
    if (StaticLoadClass) resolved++;
    if (StaticConstructObject) resolved++;
    if (GetTransientPackage) resolved++;
    if (PakMount) resolved++;
    if (GUObjectArray) resolved++;
    if (GNames) resolved++;
    if (GEngine) resolved++;
    if (GWorld) resolved++;
    if (FName_Init) resolved++;
    if (FOutputDevice_Log) resolved++;
    if (FText_ToString) resolved++;
    if (FText_FromString) resolved++;
    if (FText_Dtor) resolved++;
    if (FText_Ctor) resolved++;
...
// Find the on-disk path of libUE4.so by reading /proc/self/maps
...
static int read_elf_symbols(const uint8_t* elf_base, size_t elf_size,
    ...
    if (!sym_shdr || !str_shdr) return 0;
    if (sym_shdr->sh_offset + sym_shdr->sh_size > elf_size) return 0;
    if (str_shdr->sh_offset + str_shdr->sh_size > elf_size) return 0;
    ...
    for (size_t i = 0; i < num_syms; i++) {
        ...
        if (sym.st_name == 0) continue;  // skip unnamed
        if (sym.st_name >= strtab_size) continue;  // bounds check
...
int dump_symbols(const std::string& output_path) {
    ...
    if (lib_path.empty()) {
        logger::log_error("SYMDUMP", "Cannot find libUE4.so path in /proc/self/maps");
        return -1;
    }
    ...
    for (int i = 0; i < ehdr->e_shnum; i++) {
        ...
        if (strcmp(name, ".dynsym") == 0)  dynsym_shdr = &shdrs[i];
        if (strcmp(name, ".dynstr") == 0)  dynstr_shdr = &shdrs[i];
        if (strcmp(name, ".symtab") == 0)  symtab_shdr = &shdrs[i];
        if (strcmp(name, ".strtab") == 0)  strtab_shdr = &shdrs[i];
    }
    ...
    if (!out) {
        logger::log_error("SYMDUMP", "Cannot open output file: %s", output_path.c_str());
        return -1;
    }

    fprintf(out, "# libUE4.so Symbol Dump\n");
    ...
    for (const auto& sym : all_symbols) {
        const char* section;
        if (sym.shndx == SHN_UNDEF)      section = "UNDEF";
        else if (sym.shndx == SHN_ABS)    section = "ABS";
        else if (sym.shndx == SHN_COMMON) section = "COMMON";
        else                              section = "DEFINED";
        ...
        if (sym.shndx != SHN_UNDEF) defined_count++;
    }
