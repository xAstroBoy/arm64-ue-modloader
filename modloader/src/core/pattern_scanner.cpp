...
void init() {
    ...
    char line[512];
    bool found_libue4 = false;
    while (fgets(line, sizeof(line), maps)) {
        // Only care about libUE4.so regions
        ...
        if (region.executable) {
            if (s_text_start == 0 || start < s_text_start) s_text_start = start;
            if (end > s_text_end) s_text_end = end;
        } else if (region.readable && region.writable) {
            if (s_data_start == 0 || start < s_data_start) s_data_start = start;
    ...
    if (found_libue4) {
        logger::log_info("PATTERN", "libUE4.so mapped: .text 0x%lX-0x%lX, .data 0x%lX-0x%lX (%zu regions)",
                         s_text_start, s_text_end, s_data_start, s_data_end, s_regions.size());
    } else {
        logger::log_error("PATTERN", "libUE4.so not found in /proc/self/maps");
    }
}
...
static void* scan_region(uintptr_t start, uintptr_t end, const std::vector<PatternByte>& bytes) {
    ...
    if (end - start < pat_len) return nullptr;
...
void* scan(const std::string& pattern_str) {
    ...
    // Scan all readable regions of libUE4.so
...
std::vector<void*> scan_all(const std::string& pattern_str) {
    ...
    for (const auto& region : s_regions) {
        if (!region.readable) continue;
        size_t pat_len = bytes.size();
        if (region.end - region.start < pat_len) continue;
