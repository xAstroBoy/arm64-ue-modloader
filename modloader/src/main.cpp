...
static void* early_pak_thread(void* /*arg*/) {
    ...
    while (attempts < max_attempts) {
        ...
        if (base != 0) {
            // Also try to get a dlsym-usable handle
    ...
    if (base == 0) {
        __android_log_print(ANDROID_LOG_ERROR, LOG_TAG,
            "[PAK-EARLY] libUE4.so NOT found after %d attempts (%.1fs) — early hooks FAILED",
            attempts, attempts * 0.01f);
        return nullptr;
    }

    __android_log_print(ANDROID_LOG_INFO, LOG_TAG,
        "[PAK-EARLY] libUE4.so found after %d attempts (%.0fms) — base=0x%lX, handle=%p",
...
static void modloader_constructor() {
    __android_log_print(ANDROID_LOG_INFO, LOG_TAG,
