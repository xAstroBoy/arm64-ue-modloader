...
// ═══ UStruct offsets (RE4 VR — shifted +0x10 from default) ══════════════
namespace ustruct {
    constexpr uint32_t SUPER_STRUCT      = 0x40;
    constexpr uint32_t CHILDREN          = 0x48;  // UField* chain (functions)
    constexpr uint32_t CHILD_PROPERTIES  = 0x50;  // FField* chain (properties, UE4.23+)
    constexpr uint32_t PROPERTIES_SIZE   = 0x58;
}

// ═══ UFunction offsets (RE4 VR) ═════════════════════════════════════════
namespace ufunc {
    constexpr uint32_t FUNCTION_FLAGS     = 0xB0;
    constexpr uint32_t NUM_PARMS          = 0xB4; // uint8 — number of params
    constexpr uint32_t PARMS_SIZE         = 0xB6; // uint16 — total params buffer size
    constexpr uint32_t RETURN_VALUE_OFFSET= 0xB8; // uint16 — offset of return val in params
    constexpr uint32_t FUNC_PTR           = 0xD8; // void* — native function pointer
}
...
// ═══ FField offsets (UE4.23+ / RE4 VR) ═════════════════════════════════
namespace ffield {
    constexpr uint32_t CLASS_PRIVATE = 0x08;  // FFieldClass*
    constexpr uint32_t OWNER         = 0x10;  // FFieldVariant (UStruct* or FField*)
    constexpr uint32_t NEXT          = 0x20;  // FField*
    constexpr uint32_t NAME_PRIVATE  = 0x28;  // FName
}

// ═══ FProperty offsets (RE4 VR — shifted) ═══════════════════════════════
namespace fprop {
    constexpr uint32_t ELEMENT_SIZE    = 0x38;
    constexpr uint32_t PROPERTY_FLAGS  = 0x40;
    constexpr uint32_t OFFSET_INTERNAL = 0x4C;
    constexpr uint32_t SIZE            = 0x78;  // approximate base FProperty size

    // UBoolProperty extras
    constexpr uint32_t BOOL_FIELD_SIZE = 0x78;
    constexpr uint32_t BOOL_BYTE_OFFSET= 0x79;
    constexpr uint32_t BOOL_BYTE_MASK  = 0x7A;
    constexpr uint32_t BOOL_FIELD_MASK = 0x7B;

    // Typed property inner pointers
    constexpr uint32_t OBJ_PROPERTY_CLASS   = 0x78;
    constexpr uint32_t CLASS_META_CLASS     = 0x80;
    constexpr uint32_t INTERFACE_CLASS      = 0x80;
    constexpr uint32_t ARRAY_INNER          = 0x78;
    constexpr uint32_t MAP_KEY_PROP         = 0x78;
    constexpr uint32_t MAP_VALUE_PROP       = 0x80;
    constexpr uint32_t SET_ELEMENT_PROP     = 0x78;
    constexpr uint32_t STRUCT_INNER_STRUCT  = 0x78;
    constexpr uint32_t ENUM_PROP_ENUM       = 0x80;
    constexpr uint32_t BYTE_PROP_ENUM       = 0x78;
}
...
using StaticFindObjectFn = UObject*(*)(UClass* cls, UObject* outer, const wchar_t* name, bool exact);

// ═══ StaticLoadObject signature ═════════════════════════════════════════
using StaticLoadObjectFn = UObject*(*)(UClass* cls, UObject* outer, const wchar_t* name,
                                        const wchar_t* filename, uint32_t load_flags,
                                        void* sandbox, bool allow_obj_redirect);

// ═══ StaticLoadClass signature ══════════════════════════════════════════
using StaticLoadClassFn = UClass*(*)(UClass* base_cls, UObject* outer, const wchar_t* name,
                                      const wchar_t* filename, uint32_t load_flags, void* sandbox);
...
inline UStruct* ustruct_get_super(const UStruct* s) {
    return read_field<UStruct*>(s, ustruct::SUPER_STRUCT);
}

inline UField* ustruct_get_children(const UStruct* s) {
    return read_field<UField*>(s, ustruct::CHILDREN);
}

inline FField* ustruct_get_child_properties(const UStruct* s) {
    return read_field<FField*>(s, ustruct::CHILD_PROPERTIES);
}

inline int32_t ustruct_get_properties_size(const UStruct* s) {
    return read_field<int32_t>(s, ustruct::PROPERTIES_SIZE);
}
...
inline bool is_valid_uobject(const UObject* obj) {
    if (!is_valid_ptr(obj)) return false;
    auto cls = uobj_get_class(obj);
    if (!is_valid_ptr(cls)) return false;
...
inline bool is_default_object(const char* name) {
    ...
    if (strncmp(name, "Default__", 9) == 0) return true;
    if (strncmp(name, "REINST_",   7) == 0) return true;
    if (strncmp(name, "SKEL_",     5) == 0) return true;
    return false;
}
