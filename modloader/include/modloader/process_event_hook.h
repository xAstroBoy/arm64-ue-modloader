...
namespace pe_hook
{
    ...
    // Invoke ProcessEvent on the game thread and wait for completion.
    // If called from the game thread, executes immediately.
    // Returns false on timeout or if ProcessEvent is unavailable.
    bool invoke_game_thread_sync(ue::UObject *self,
                                 ue::UFunction *func,
                                 void *parms,
                                 const char *tag = "PE",
                                 const char *op_name = "ProcessEvent",
                                 int timeout_ms = 8000);
